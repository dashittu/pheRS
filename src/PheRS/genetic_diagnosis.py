...
def get_dx_status(platform='aou', demos_path=None, icd_occurrences=None, min_unique_ages=2, disease_dx_icd_map=None):
    """
    ...
    dx_icd_unique_ages = dx_icd.groupby(['person_id', 'disease_id']).agg(pl.count('occurrence_age').alias('uniq_ages'))
    ...
    }).join(pl.DataFrame(disease_dx_icd_map['disease_id'].unique().repeat(len(demos['person_id']))), on='person_id',
            how='cross')
...
if __name__ == "__main__":
    dx_status = get_dx_status(platform='custom', demos_path='/Users/dayoshittu/Downloads/demo_data_sample.csv',
                              icd_occurrences=None, min_unique_ages=2)
