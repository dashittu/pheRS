...
class Weights:
    ...
    def get_weights_logistic(self, phecode_occurrences_path=None, method_formula=None, negative_weights=False,
                             n_jobs=1, output_file_name=None, chunk_size=1000):
        start_time = datetime.now()
        ...
        def logistic_regression_chunk(chunk_index):
            ...
            def logistic_regression(phe):
                ...
                glm_input = demo_chunk.join(phe_sub.with_columns(pl.lit(phe).alias('phecode')), on='person_id', how='left')
            ...
            with ThreadPoolExecutor(max_workers=self.n_threads) as executor:
                ...
                for future in tqdm(as_completed(futures), total=len(unique_phecodes), desc=f"Chunk {chunk_index} Logistic Regression"):
        ...
        for chunk_index in range(num_chunks):
            ...
            chunk_weights_dfs = logistic_regression_chunk(chunk_index)
    ...
    def get_weights_loglinear(self, phecode_occurrences_path=None, method_formula=None, negative_weights=False,
        ...
        def loglinear_regression_chunk(chunk_index):
            ...
            with ThreadPoolExecutor(max_workers=self.n_threads) as executor:
                ...
                for future in tqdm(as_completed(futures), total=len(unique_phecodes), desc=f"Chunk {chunk_index} Loglinear Regression"):
                    if self.suppress_warnings:
                        with warnings.catch_warnings():
                            warnings.simplefilter("ignore")
                            weight_result = future.result()
        ...
        for chunk_index in range(num_chunks):
            print(f"Processing chunk {chunk_index + 1} out of {num_chunks}")
            chunk_weights_dfs = loglinear_regression_chunk(chunk_index)
            weights_dfs.extend(chunk_weights_dfs)
    ...
    def get_weights_cox(self, phecode_occurrences_path=None, method_formula=None, negative_weights=False, n_jobs=1,
        ...
        def cox_regression_chunk(chunk_index):
            demo_chunk = demos.slice(chunk_index * chunk_size, chunk_size)

            def cox_regression(phe):
                phe_sub = phecode_occurrences.filter(pl.col('phecode') == phe).select(
                    ['person_id', 'occurrence_age']).unique()
                cox_input = demo_chunk.join(phe_sub, on='person_id', how='left').with_columns(
            ...
            with ThreadPoolExecutor(max_workers=self.n_threads) as executor:
                ...
                for future in tqdm(as_completed(futures), total=len(unique_phecodes), desc=f"Chunk {chunk_index} Cox Regression"):
                    if self.suppress_warnings:
                        with warnings.catch_warnings():
                            warnings.simplefilter("ignore")
                            weight_result = future.result()
        ...
        for chunk_index in range(num_chunks):
            print(f"Processing chunk {chunk_index + 1} out of {num_chunks}")
            chunk_weights_dfs = cox_regression_chunk(chunk_index)
            weights_dfs.extend(chunk_weights_dfs)
